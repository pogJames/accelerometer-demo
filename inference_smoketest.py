"""inference_smoketest.py — verify the backbone .tflite works in isolation
...
Usage:
    python inference_smoketest.py                   # NPU (fall back to CPU)
    FORCE_CPU=1 python inference_smoketest.py       # CPU only
    python inference_smoketest.py --runs 500        # more samples for stable timing
    python inference_smoketest.py --compare         # run both NPU and CPU, print both
"""
...
def build_interpreter(model_path, use_npu):
    ...
    try:
        ...
        sys.exit(f"ai_edge_litert not available: {e}")
